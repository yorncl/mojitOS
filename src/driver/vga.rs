...
static mut VGA_INSTANCE: Option<RwLock<VGA>> = None;
...
pub fn io_init() {
    unsafe {
        VGA_INSTANCE = Some(RwLock::new(VGA::new()));
        let mut buffer = VGA_INSTANCE.as_mut().unwrap().write().unwrap();
        buffer.deref_mut().clear();
        // r.deref_mut().clear();
    }
}
...
impl VGA {
    ...
    fn new_line(&mut self) {
        ...
        if self.y == HEIGHT {
            ...
            mem::memset(
                self.buffer.data[HEIGHT - 1].as_ptr() as *mut c_void,
                0,
                WIDTH * 2,
            );
            self.y = HEIGHT - 1;
