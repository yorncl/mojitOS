...
fn parse_memory_map(info: &MultibootInfo) {
    ...
    for _ in 0..nentries {
        unsafe {
            ...
            memory::phys_mem().add_entry(memory::PhysicalRegion::new(
                entry.addr as usize,
                entry.len as usize,
                entry.type_ as usize,
            ));
            ptr = ptr.offset(1);
        }
    }
...
pub fn parse_mboot_info(ptr: *const u32) -> Result<()> {
